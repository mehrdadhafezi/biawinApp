...
#   4. the default catalog seed   ($DEFAULT_CATALOG_CMD, sourced from deploy.sh below)
#
...
# migration/default-catalog commands. It `source`s deploy.sh itself (with
# DEPLOY_SH_SOURCE_ONLY=1, so deploy.sh only defines its SEED_CMD /
# MEDIA_MIGRATION_CMD / DEFAULT_CATALOG_CMD variables and returns, touching
# nothing else) and runs those exact strings. An earlier version hardcoded
# its own copy here, and
...
# deploy.sh, prisma/seed.ts, prisma/seed-home-media.ts, prisma/seed-default-
# catalog.ts, or anything they import:
#
...
log "0/6 static check: deploy.sh must not invoke prisma db seed / ts-node against prisma source"
...
if grep -vE '^[[:space:]]*#' "$DEPLOY_SH" | grep -qE 'prisma[[:space:]]+db[[:space:]]+seed|ts-node.*prisma/(seed|seed-home-media|seed-default-catalog)\.ts'; then
  fail "deploy.sh contains a banned pattern (prisma db seed / ts-node against prisma/*.ts source) in executable code — this is exactly the bug that broke real staging. Run: grep -vE '^[[:space:]]*#' $DEPLOY_SH | grep -nE 'prisma db seed|ts-node.*prisma/(seed|seed-home-media|seed-default-catalog)\\.ts'"
fi

log "0/6 sourcing SEED_CMD / MEDIA_MIGRATION_CMD / DEFAULT_CATALOG_CMD from deploy.sh itself (not a hand-copied duplicate)"
...
[ -n "${DEFAULT_CATALOG_CMD:-}" ] || fail "deploy.sh did not define \$DEFAULT_CATALOG_CMD when sourced — did its variable name change without this script being updated?"
log "  SEED_CMD=$SEED_CMD"
log "  MEDIA_MIGRATION_CMD=$MEDIA_MIGRATION_CMD"
log "  DEFAULT_CATALOG_CMD=$DEFAULT_CATALOG_CMD"

log "1/6 tearing down any leftover verification stack from a previous run"
"${COMPOSE[@]}" down -v --remove-orphans >/dev/null 2>&1 || true

log "2/6 building the real staging backend image (deploy/staging/Dockerfile.backend)"
"${COMPOSE[@]}" build backend

log "3/6 starting throwaway postgres/redis/minio and waiting for health"
...
log "4/6 running \$SEED_CMD against the real image (this is deploy.sh's own step 4/8, verbatim)"
...
log "5/6 running \$MEDIA_MIGRATION_CMD against the real image (this is deploy.sh's own step 5/8, verbatim)"
...
log "6/6 running \$DEFAULT_CATALOG_CMD against the real image (this is deploy.sh's own step 6/8, verbatim)"
if ! "${COMPOSE[@]}" run --rm backend sh -c "$DEFAULT_CATALOG_CMD"; then
  fail "DEFAULT_CATALOG_CMD failed inside the real runtime image — same class of bug as above (it needs the real Nest application context/MediaService, exactly like MEDIA_MIGRATION_CMD, and must run after SEED_CMD's Category/Service/SUPER_ADMIN rows exist)."
fi
